...
const map<const string, Layer* (*)(picojson::object&, const string&, OptOption&)> BUILD_LAYER_MAP{
 ...
 */
Layer* buildLayer(picojson::object& layer, 
                  ...
                  OptOption& option)
{
    ...
    if (layer_class_name == CONV2D_CLASS_NAME && next_layer_class_name == BATCH_NORMALIZATION_CLASS_NAME) {
        ...
        layers_iterator++;
        return move(buildConv2DFusedBN(conv2d_layer_info, bn_layer_info, model_weights_path, option));
    }
    if (layer_class_name == DENSE_CLASS_NAME && next_layer_class_name == BATCH_NORMALIZATION_CLASS_NAME) {
        ...
        picojson::object bn_layer_info    = next_layer["config"].get<picojson::object>();
        layers_iterator++;
        return move(buildDenseFusedBN(dense_layer_info, bn_layer_info, model_weights_path, option));
    }

    return move(buildLayer(layer, layer_class_name, model_weights_path, option));
}
                                   ...
                                   OptOption& option)
{
    ...
    if (true || option.enable_optimize_pooling) {
        if (option.should_multiply_pool) {
            option.current_pooling_mul_factor *= (1.0 / (next_layer_in_height * next_layer_in_width));
        } else {
            option.current_pooling_mul_factor = 1.0 / (next_layer_in_height * next_layer_in_width);
        }
        option.should_multiply_pool = true;
