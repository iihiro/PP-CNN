...
void callback_func(const int32_t query_id,
                   const bool status,
                   const std::vector<seal::Ciphertext>& enc_results, 
                   void* args)
{
    ...
    std::vector<vector<double>> results(param->img_count, vector<double>(param->labels));
    ...
    for (size_t i=0; i<param->labels; ++i) {
        ...
        for (size_t j=param->img_beg_idx, counter=0; j<param->img_end_idx; ++j) {
    ...
    for (size_t step = 0, img_count_in_step; step < step_count; ++step) {
        ...
        for (size_t n = 0; n < number_prediction_trials; ++n) {
            ...
            encryptImages(test_imgs, enc_packed_imgs, 
                          beg_idx, end_idx, scale_param, *encryptor, *encoder);

            auto elem_num = rows * cols * channels;
            ppcnn_share::EncData enc_inputs(enc_params, enc_packed_imgs.data(), elem_num);
...
void exec(Option& option)
{
    ...
    auto key_id = init_keys(option.config_filepath, seckey, pubkey, relinkey, enc_params);
    ...
    auto context = seal::SEALContext::Create(enc_params);
    std::shared_ptr<seal::Decryptor> decryptor(new seal::Decryptor(context, seckey));
    std::shared_ptr<seal::CKKSEncoder> encoder(new seal::CKKSEncoder(context));
